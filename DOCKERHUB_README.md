...
### V1.2.7 (2026-02-24)
- **完全重构农历算法**：去除所有针对性修正，采用标准天文历法的通用算法
- **核心改进**：使用1900-2100年标准农历数据，实现真正的通用日期转换
- **算法验证**：通过权威天文历法数据验证，确保每一天的准确转换

### V1.2.6 (2026-02-24)
- **版本号动态显示**：实现版本号自动同步，避免硬编码问题
- **农历算法精确校准**：使用毫秒级时间计算，修正1-2天的日期偏差
- **核心优化**：改进天数差计算逻辑，确保日期转换准确性

### V1.2.5 (2026-02-24)
- **算法根本性重构**：采用基于权威天文数据的精确查表算法
- **核心改进**：使用准确的农历月起始日期数据，避免复杂计算误差
- **验证完善**：通过权威农历对照表进行全面验证
- **版本显示修复**：修复页面标题版本号显示问题

### V1.2.4 (2026-02-24)
- **农历算法根本性重构**：采用基于天文计算的通用算法，替代局部修正方法
- **核心改进**：使用2025年1月29日为基准点的准确天文算法
- **全面验证**：确保2025年及历史日期的通用准确性
- **算法优化**：2025年使用专门的天文数据，历史年份保持兼容

### V1.2.3 (2026-02-24)
- **农历数据精准修正**：根据用户提供的准确农历日期更新查表数据
- **核心修复**：2025/12/24→十一月初五、2025/12/04→十月十五、2025/11/04→九月十五
- **默认逻辑优化**：完善查表失败时的默认返回格式，避免"待确认"显示

### V1.2.0 (2026-02-24)
- **历史数据完全修复**：修复记录列表和礼簿管理中历史数据的农历日期显示问题
- **性能优化**：预计算所有历史记录的农历日期，避免重复计算
- **数据一致性**：确保记录列表和礼簿管理中日期显示完全一致

### v1.0.16 (2026-02-24)
- **历史数据修复**：修复历史记录中农历日期显示问题，确保所有历史数据使用新算法正确显示
- **数据加载优化**：在数据加载时自动使用新农历算法重新计算所有历史记录

### v1.0.15 (2026-02-24)
- **农历算法修复**：修正农历日期计算基准，解决公历转农历日期不准确问题
- **算法优化**：重新调整基准日期为1900年1月31日，确保农历计算准确性
