...
- 👥 **智能输入**: 姓名联想、自动填充
- 📊 **统计分析**: 年度统计、事由分析
- 🔐 **安全增强**: 密码哈希存储、登录会话管理、登录限流防爆破；默认密码 admin，首次登录提示修改
...
# 删除密码文件（容器内对应 /app/data/password.json）
rm password.json

# 重启容器
docker-compose restart
```
应用重启后会自动生成默认密码 `admin`（首次登录会再次提示修改）。

> ⚠️ 注意：此操作会丢失所有已保存的密码，但不会影响人情记录和礼薄数据。
...
- **安全与认证增强**
  - 增加基础安全能力：登录会话校验、密码哈希存储、登录限流防爆破
  - 修复默认密码登录异常与登录后回跳登录页问题
- **中文与文案一致性修复**
...
- **发布与运维规范化**
  - 版本基线统一为 `2.0.0`
  - 发布仓库统一为 `zsw01442/liji`
  - 镜像支持多架构发布：`linux/amd64` + `linux/arm64`
